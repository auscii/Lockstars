    ...
    controller.scannedDataStream.listen((scanData) {
      setState(() async {
        ...
        if (result!.code.contains("http://lockstarsph.com/l/")) {
          String selLocker = "${result!.code.substring(25)}";
          print(
              "${selLocker} is ${await _db.getTransactionField(selLocker, "status")}");
          if (transactionData.selLocker == selLocker) {
            //target is a match
            Navigator.pop(context);
            print("unlocking...");
            _db.updateTransactField(transactionData.selLocker, "command",
                "OPEN,${transactionData.seldecUnlockCode},${transactionData.selChargeHour}");
            Navigator.pop(context);
          } else {
  ...
  Future<void> showAlert(BuildContext context) async {
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: new Text("Incorrect QR"),
          actions: <Widget>[
            Text(
                "To unlock ${transactionData.selLocker}, kindly scan QR code on the locker door."),
            TextButton(
              child: new Text("OK"),
              onPressed: () {
